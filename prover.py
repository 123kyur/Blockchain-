...
def gen_merkle_proof(leaves, pos):
    ...
    if pos % 2 == 0:            #verify it is left or child node and get its sibling add to path 
        path.append(state[pos+1])
    else:
        path.append(state[pos-1])
        
    for level in range(height-1):
        level_pos = int(level_pos / 2)          #get its parent
        for i in range(0,2**(height-level),2):  #update this level node
            state[int(i/2)]=hash_internal_node(state[i],state[i+1])
        if level_pos%2==0:                      #verify it is left or child node and get its sibling add to path
